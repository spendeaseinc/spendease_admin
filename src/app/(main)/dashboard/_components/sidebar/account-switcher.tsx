...
export function AccountSwitcher({ user }: AccountSwitcherProps) {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger>
        <Avatar className="border-primary size-9 rounded-none border">
          <AvatarImage src={displayUser.avatar} alt={displayUser.name} />
          <AvatarFallback className="rounded-none">{getInitials(user.name)}</AvatarFallback>
          <AvatarImage className="rounded-none">{getInitials(displayUser.name)}</AvatarImage>
        </Avatar>
      </DropdownMenuTrigger>
      <DropdownMenuContent className="min-w-56 space-y-1 rounded-none" side="bottom" align="end" sideOffset={4}>
        <DropdownMenuItem
          key={displayUser.email}
          className={"bg-accent/50 border-l-primary rounded-none border-l-2 p-0"}
        >
